...
uses
  Classes, SysUtils, Controls, Windows, Messages, ActiveX, ComObj, Forms, Graphics;
...
type
  ...
  lprect=^TRect;
  PRect=LPRECT;
  LPOLEINPLACEFRAMEINFO=^TOLEImplaceFrameInfo;
  HRESULT=integer;
  wndproc=function(dlg,msg,wparam,lparam:integer):integer;stdcall;
  ...
  TActiveXContainer = class(TWinControl, IUnknown, IOleClientSite,
    ...
    private
      ...
      function TranslateAccelerator(var msg: TMsg; wID: Word): HResult;overload;StdCall;
      //IOleInPlaceUIWindow
      ...
      function SetBorderSpace(borderwidths: PRect):HResult;StdCall;
...
begin
  AXC:=TActiveXContainer(integer(GetWindowLong{PtrW}( Ahwnd, GWL_USERDATA{P})));
  case uMsg of
...
begin
  if FAttached then
    begin
    SetWindowLong{Ptr}(Handle,GWL_WNDPROC,PtrInt(@FPrevWndProc));
    SetWindowLong{Ptr}(Handle,GWL_USERDATA, 0); {P}
    end;
